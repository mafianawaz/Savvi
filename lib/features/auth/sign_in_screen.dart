...
/// Sign-in entry screen. Returning members sign in with email + password;
/// new members are routed to the nonprofit access-link flow. Firebase Auth
/// is wired in a later stage; the auth controller currently drives the flow
/// via the mock session call so navigation and states are real.
class SignInScreen extends StatelessWidget {
  const SignInScreen({super.key});
...
/// Small language switch (en-US / es-US), consistent with the app-wide
/// locale. Logic-free — reads the controller's Rx locale directly via Obx.
class LanguageToggle extends StatelessWidget {
  const LanguageToggle({super.key});
