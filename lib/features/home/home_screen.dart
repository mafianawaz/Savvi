...
/// Home dashboard: member line + greeting + tagline, a photo request CTA
/// card, two photo navigation tiles (Events, My Requests), and the
/// active-request preview. No monetary or Delivery Credit content.
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      // TODO(notifications): wire to NotificationsController.unreadCount
      // once the Notifications feature is migrated to GetX. Defaulting to
      // 0 keeps the badge hidden rather than showing a stale/wrong count.
      const unread = 0;
        ...
        child: ListView(
          ...
          children: [
            ...
            if (active != null)
              RequestCard(
                request: active,
                // NOTE: Routes.requestDetail is a static path (no :id
                // segment) and RequestDetailScreen itself is still on
                // Riverpod (Activity feature, not yet migrated). Passing
                // the id via Get.arguments matches the convention already
                // used for Routes.approval elsewhere in app_router.dart.
                onTap: () => Get.toNamed(
                  Routes.requestDetail,
                  arguments: active.id,
                ),
  ...
  /// Photo CTA — a food photo fills the card with a navy gradient scrim
  /// behind the title/body/button so text stays legible, matching the v56
  /// reference's image-forward hero treatment (previously a flat navy
  /// card with no imagery).
  Widget _ctaCard(
      BuildContext context,
      AppLocalizations l,
      String nonprofit,
              ...
              DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [
                      SavColors.navy.withOpacity(0.15),
                      SavColors.navy.withOpacity(0.92),
                    ],
                  ),
                ),
              ),
  ...
  /// Photo tiles — Events uses the food-access-alert photo, My Requests
  /// uses the request photo, each with the same navy scrim + white text
  /// treatment as the CTA so Home reads as one consistent, photo-led
  /// design instead of mixing photo hero + flat icon tiles.
  Widget _tiles(AppLocalizations l) {
    return Row(
      children: [
        Expanded(
            ...
            children: [
              Image.asset(
                ...
                errorBuilder: (context, error, stack) => Container(
                  ...
                  child: Icon(icon, size: 26, color: SavColors.navy),
                ),
              ),
              DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [
                      SavColors.navy.withOpacity(0.05),
                      SavColors.navy.withOpacity(0.85),
                    ],
                  ),
                ),
              ),
