                ...
                children: [
                  Center(
                    child: Container(
                      ...
                      child: const Icon(Icons.mail_outline, color: SavColors.pillGreenFg, size: 30),
                    ),
                  ),
                  ...
                  SavButton(
                    label: l.iHaveTheLink,
                    onPressed: () => controller.iHaveTheLink(email),
                  ),
                  const SizedBox(height: SavSpace.x10),
                  Obx(() {
                    final cooldown = controller.resendCooldown.value;
                    return SavButton(
                      label: cooldown > 0 ? l.resendInSeconds(cooldown) : l.resendLink,
                      variant: SavButtonVariant.ghost,
                      busy: controller.isSubmitting.value,
                      onPressed: controller.canResend ? () => controller.resendLink(context, email) : null,
                    );
                  }),
                  const SizedBox(height: SavSpace.x10),
                  SavButton(
